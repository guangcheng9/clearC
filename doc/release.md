...
当前发布基础来自 `V1 / M7：打包发布准备`，目标是验证 Windows 桌面应用可以构建，并形成安装包发布流程。当前功能进度已进入 `V2` 收口核对阶段，安装包完善仍归入 `V4 / M17`。
...
- `doc/` 文档已同步当前功能边界。
- 清理功能不执行永久删除。
- 用户目录迁移、开发工具环境变量迁移和 Junction 迁移必须只在用户点击确认后执行。
- 发布前必须确认真实迁移、环境变量写入、Junction 创建与回滚的手动验收状态。

## 当前限制

- 当前真实清理仅支持 `%TEMP%` 隔离移动。
- 用户目录迁移执行和回滚代码已实现，正式发布前需要完成真实手动验收。
- 开发工具环境变量迁移和回滚代码已实现，正式发布前需要完成真实手动验收。
- 开发工具 Junction 真实执行和回滚代码已实现，正式发布前需要在测试目录中完成真实手动验收。
- 便携版 zip 需要后续补脚本生成。
